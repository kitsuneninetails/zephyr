...
class UpgradeTestBase(test_case.TestCase):
    ...
    def upgrade_test_main_flow(self, underlay_module, new_version,
        ...
        mycli = cli.LinuxCLI()
        mycli.add_environment_variable("VERSION", new_version)
        mycli.add_environment_variable("API_PORT", midonet_api_port)
        mycli.add_environment_variable("PASSWORD", midonet_password)
        mycli.add_environment_variable("ZK_SERVERS", zkserver)
        mycli.cmd(ROOT_DIR + '/zephyr/midonet/upgrade-script.sh')
